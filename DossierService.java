...
public class DossierDataServiceImpl implements DossierDataService {
	...
	static {
		...
		List<String> fields = Arrays.asList(
				...
				DossierDataDto.Fields.propertyData+ObjectUtils.PROPERTY_NAME_SEPARATOR+ PropertyDataDto.Fields.properties,
				DossierDataDto.Fields.propertyData+ObjectUtils.PROPERTY_NAME_SEPARATOR+ PropertyDataDto.Fields.coFinancing,
				DossierDataDto.Fields.beneficiaries);
	...
	public DossierDataDto patchDossier(DossierDataDto dossierDataDto) {
		...
		Assert.contains(DossierStatus.getStatusesPatchDossier(),oldDossier.getCodeStatus(), messageService.getMessage(API_ACTION_NOT_ALLOWED));
		...
		if (isDelayedNull && DossierStatus.statusesToPurgeDelayedFields().contains(oldDossier.getCodeStatus()) ){
	...
	private boolean checkIfUserIsFirstDeliveryManger(DossierDataCoreDto dossier) {
		...
		return 	!CollectionUtils.isEmpty(assignmentUnitsList) &&
	...
	public void generateDefaultWarranties(DossierDataCoreDto dossier) {
		...
		addPropertyWarranties(dossier, defaultWarranties, hasEmprunteur, hasOtherBeneficiaries);
	...
	private void addPropertyWarranties(DossierDataCoreDto dossier, List<WarrantyDto> list, boolean hasEmprunteur, boolean hasOtherBeneficiaries) {

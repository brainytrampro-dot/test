...
import ma.sg.its.octroicredit.common.dto.UserDto;
...
import ma.sg.its.octroicreditapi.enumeration.Role;
import ma.sg.its.octroicreditapi.enumeration.WorkflowValidationResult;
...
@Service("application_assignment")
public class ApplicationAssignmentTaskCompletion extends DossierWorkflowTaskCompletion<WorkflowTaskCompletionDto> {
  ...
  public void execute(TaskResult task, DossierDataCoreDto dossier, WorkflowTaskCompletionDto workflowTaskCompletionDto) {
    Assert.equals(DossierStatus.DSC_OPC_GENERATION_PROCESSING.getCode(), dossier.getCodeStatus(),
        messageService.getMessage(MessageKeyConstants.API_ACTION_NOT_ALLOWED_STATUS_NOT_VALID));
    UserDto bankProcessingCharger = (UserDto) workflowTaskCompletionDto.getObject();
    Map<String, String> variables = new HashMap<>();
    dossierDataHelper.purgeNonUpdatableDossierFields(dossier);
    dossierDataHelper.fillDossierUser(dossier, Role.CTB, bankProcessingCharger);
    variables.put(WorkflowConstants.BANK_PROCESSING_OFFICER_VARIABLE_NAME, bankProcessingCharger.getIdentifier());
    variables.put(WorkflowConstants.CLIENT_DECISION_RESULT, WorkflowValidationResult.OK.toString());
    dossier.setCodeStatus(DossierStatus.DSC_OPC_GENERATION_PROCESSING_CTB.getCode());

    ppWorkflowService.completeTask(task, variables, dossier);
  }

  @Override
  public String getAssigneeVariable() {
    return WorkflowConstants.DELIVERY_MANAGER_VARIABLE_NAME;
  }
...
import com.sgma.ms.bpm.client.model.ActivateProcessResult;
import com.sgma.ms.bpm.client.model.StartProcessResult;
import com.sgma.ms.bpm.client.model.TaskResult;
...
import ma.sg.its.octroicredit.common.dto.NotificationContext;
...
import ma.sg.its.octroicredit.common.service.ReferentialService;
import ma.sg.its.octroicredit.common.service.UserService;
import ma.sg.its.octroicredit.common.service.WorkflowService;
import ma.sg.its.octroicredit.common.service.WorkflowTaskCompletion;
import ma.sg.its.octroicreditapi.client.DossierDataClient;
import ma.sg.its.octroicreditapi.client.TaskClient;
import ma.sg.its.octroicreditapi.config.DossierSyncFlow;
import ma.sg.its.octroicreditapi.constant.WorkflowConstants;
import ma.sg.its.octroicreditapi.dto.DossierDataDto;
import ma.sg.its.octroicreditapi.dto.TaskDto;
import ma.sg.its.octroicreditapi.dto.UpdateDossierAndTaskRequest;
import ma.sg.its.octroicreditapi.dto.core.DossierDataCoreDto;
import ma.sg.its.octroicreditapi.enumeration.DossierStatus;
import ma.sg.its.octroicreditapi.enumeration.Role;
import ma.sg.its.octroicreditapi.helper.DossierDataHelper;
import ma.sg.its.octroicreditapi.mapper.DossierUserDtoMapper;
import ma.sg.its.octroicreditapi.service.PPWorkflowService;
import ma.sg.its.octroicreditapi.strategies.*;
import ma.sg.its.octroicreditapi.strategies.agencyNotif.NotifyDADossierStrategy;
import ma.sg.its.octroicreditapi.strategies.agencyNotif.NotifyDAFODossierStrategy;
...
public class PPWorkflowServiceImpl implements PPWorkflowService {
    ...
    private TaskResult getFilteredTask(List<TaskResult> tasks) {
        ...
        boolean isInitiator = Arrays.asList("CCP", "CCPPRO", "CCPBG", "DA").contains(codeProfession);
...
public class NextTaskProcessorService {
    ...
    public TaskResult processNextTask(DossierDataCoreDto dossier) {
        log.debug("Interrogation de Camunda pour la prochaine tâche du dossier {}", dossier.getUuid());

        List<TaskResult> tasks;
        try {
            tasks = workflowService.getTasks(dossier.getUuid());
        } catch (Exception ex) {
            log.error("Erreur lors de l'appel getTasks pour le dossier {} : {}", dossier.getUuid(), ex.getMessage(), ex);
        ...
        if (newTask == null) {
            ...
            log.warn("Aucune tâche disponible pour le dossier {} — prochain essai dans quelques secondes...", dossier.getUuid());
            throw new WorkflowException(
                    dossier.getUuid(),
                    WorkflowException.WorkflowExceptionEnum.CAMUNDA_NEXT_TASK_ERROR
            );
        ...
        log.info("Prochaine tâche créée avec succès pour le dossier {} : taskId={}, activity={}",
                dossier.getUuid(), newTask.getTaskId(), newTask.getActivityName());
        return newTask;
    }

    @Recover
    public TaskResult recoverProcessNextTask(WorkflowException ex, DossierDataCoreDto dossier) {
        log.error("Échec définitif après 5 tentatives pour le dossier {}. Cause : {}",
                dossier.getUuid(), ex.getMessage());
        try {
            ...
            log.error("Impossible de mettre à jour le flowStatus du dossier {} : {}",
                    dossier.getUuid(), updateEx.getMessage(), updateEx);
        }
        return null;
    }
    ...
    private void createNextTask(DossierDataCoreDto dossier, TaskResult newTask) {
        ...
        try {
            ...
            log.warn("Impossible de créer la task en base pour le dossier {} taskId={} : {} — ignoré",
                    dossier.getUuid(), newTask.getTaskId(), ex.getMessage());
        }
    }
}

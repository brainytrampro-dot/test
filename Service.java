...
import ma.sg.its.octroicredit.common.client.ReferentialClient;
...
import ma.sg.its.octroicreditapi.strategies.NotifyCTBStrategy;
...
public class DossierDataServiceImpl implements DossierDataService {
	...
	public DossierDataDto patchDossier(DossierDataDto dossierDataDto) {
		...
		DossierDataCoreDto oldDossier = dossierDataClient.retrieveByUuid(dossierDataDto.getUuid());
		DossierDataDto dossierData = dossierDataDtoMapper.convertToApiDTO(oldDossier);
		...
		ObjectUtils.copySelectedProperties(dossierDataDto, dossierData, nonNullProperties);
		...
		if (nonNullProperties
			...
			recalculateLoanAmounts(dossierData);
		}
		if (nonNullProperties.contains(DossierDataDto.Fields.loanData + ObjectUtils.PROPERTY_NAME_SEPARATOR
			...
			recalculateRates(dossierData);
		}

		boolean isDelayedNull = (dossierDataDto.getLoanData() == null || dossierDataDto.getLoanData().getDelayed() == null);
		if (isDelayedNull && DossierStatus.statusesToPurgeDelayedFields().contains(dossierData.getCodeStatus()) ){
			dossierDataHelper.purgeDossierDelayedFields(dossierData);
		}

		return updateDossier(dossierData, dossierData.getCodeStatus());
	}
	...
	public DossierDataDto approveDossier(String uuid) throws Exception {
        ...
        oldTask.setAssignee(currentAssign != null ? currentAssign.getFullName() : null);

        TaskResult newTask = new TaskResult();
        newTask.setAssignee(currentUser != null ? currentUser.getFullName() : null);
        NotifyCTBStrategy notifyCTBStrategy = notifyStrategyFactory.getStrategy(NotifyCTBStrategy.class);
        notifyCTBStrategy.sendNotification(dossier,NotificationContext.builder().oldTask(oldTask).newTask(newTask).connectedUser(currentUser).build());
        return this.advanceDossier(dossier,
				WorkflowTaskCompletionDto.builder()
	...
	public DossierDataDto retrieveByUuid(String uuid) {
		...
		if(taskResult != null){
			...
			dossierDataDto.setTaskStatus(getTaskStatus(dossierDataDto.getCodeStatus(),taskResult,dto.getDossierAttachmentTypes()));
		}
		return dossierDataDto;
	}

private String getTaskStatus(String currentCodeStatus,
									 TaskResult taskResult, List<DossierAttachmentTypeDto> dossierAttachmentTypes) {
		...
		String taskStatus = taskStatusMap.getOrDefault(taskResult.getActivityName(), currentCodeStatus);
	...
	public SearchResponse<KpiDossierData> getAll(SearchRequest<DossierDataCriteria> searchRequest) {
		...
		if(Arrays.asList(DossierListEnum.ALL_INPROGRESS_TASKS, DossierListEnum.ALL_RETURNED_TASKS).contains(listType)) {
			if(CollectionUtils.isEmpty(params.getSearchCriteria().getInlistUuidDossier())){
	...
	public DossierDataDto generateNotificationDossier(String uuid, NotificationGeneratorDto notificationGeneratorDto)
		...
		if (dossier.getCustomerData() != null 
    		&& dossier.getCustomerData().getCustomer() != null 
    		&& !dossier.getCustomerData().getCustomer().isProspect() 
			&& AccordType.PRINCIPE.equals(dossier.getAccord()) && hasAttachment(dossier, ATTACHMENT_TYPE_NOTIFICATION)) {
	...
	public DossierDataDto releaseDossier(String uuid, ReleaseDossierDto releaseDossierDto) throws Exception {
		...
		if(!DossierStatus.PARTIAL_RELEASE_AGENCY.getCode().equals(dossier.getCodeStatus())){
			...
			if(dossier.getFirstReleasedDate() == null) dossier.setFirstReleasedDate(LocalDateTime.now());
			dossier.getLoanData().setAmountReleased(computedDto.getAmountToRelease());
		}else{
            dossier.getLoanData().setAmountToRelease(releaseDossierDto.getAmountToRelease());
        }
        dossierDataClient.update(uuid, dossier);
		return this.advanceDossier(dossier,
				WorkflowTaskCompletionDto.builder()
	...
	/**
	 * this method create new dossier request in case return instance to decision
	...
	public DossierRequestDto createDossierRequest(DossierRequestDto dossierRequestDto, DossierDataCoreDto dossier) throws IllegalArgumentException{
		...
		dossierTmp.setCreatedAt(dossier.getCreatedAt());
        ...
        dossierTmp.setBeneficiaries(dossierRequestDto.getBeneficiaries());
		dossierTmp.setPropertyData(dossierRequestDto.getPropertyData());
		dossierTmp.setGuarantors(dossierRequestDto.getGuarantors());
	...
	public DossierDataDto sendBackToDecisionInstance(String uuid, AccordType accord, DossierRequestDto dossierRequestDto) throws Exception {
		DossierDataCoreDto dossier = null;
		if(AccordType.PRINCIPE.equals(accord)) {
			DossierDataDto mappeDossier= dossierRequestMapper.toDossierDataDto(dossierRequestDto);
			mappeDossier.setUuid(uuid);
			dossier = dossierDataDtoMapper.convertToCoreDTO(patchDossier(mappeDossier));
		}else{
			dossier = dossierDataClient.retrieveByUuid(uuid);
            dossierRequestDto.setDossier(DossierDataDto.builder().uuid(dossier.getUuid()).build());
            dossierRequestDto.setStatusDossier(dossier.getCodeStatus());
            createDossierRequest(dossierRequestDto, dossier);
        }
		if(dossier == null) throw new TechnicalException("Dossier must be not null");
		return this.advanceDossier(dossier, WorkflowTaskCompletionDto.builder()
	...
	public AttachmentDto uploadCreditBureauDocument(String uuid, String attachmentTypeCode) {
		...
		try {
            ...
            markLoanBureauControlsAsConform(attachmentDto);
    ...
    private void markLoanBureauControlsAsConform(AttachmentDto attachmentDto) {
        try {
            log.info("Marking LOANBUREAU controls as conform for attachment: {}", attachmentDto.getUuid());

            RefAttachmentTypeDto refAttachmentType = referentialClient.getAttachmentTypeBy("LOANBUREAU");

            if (refAttachmentType == null || refAttachmentType.getControls() == null || refAttachmentType.getControls().isEmpty()) {
                log.warn("No controls found for LOANBUREAU attachment type");
                return;
            }

            List<RefAttachmentControlDto> controls = refAttachmentType.getControls();

            List<AttachmentControlDto> attachmentControlDtos = controls.stream()
                    .map(refControl -> AttachmentControlDto.builder()
                            .code(refControl.getCode())
                            .designation(refControl.getDesignation())
                            .conform(true)
                            .build())
                    .collect(Collectors.toList());

            attachmentDto.setControls(attachmentControlDtos);

            UserDto currentUser = dossierDataHelper.getCurrentuser();
            if (currentUser == null) {
                log.warn("Cannot mark controls as conform: current user is null");
                return;
            }

            InterventionDto interventionDto = InterventionDto.builder()
                    .userDto(currentUser)
                    .controls(attachmentControlDtos)
                    .comments(new ArrayList<>())
                    .createdAt(LocalDateTime.now())
                    .build();

            attachmentDto.setInterventions(Collections.singletonList(interventionDto));

            attachmentService.update(attachmentDto);

            log.info("All LOANBUREAU controls marked as conform successfully for attachment: {}", attachmentDto.getUuid());

        } catch (Exception e) {
            log.error("Error marking LOANBUREAU controls as conform for attachment: {}", attachmentDto.getUuid(), e);
        }
    }
	@Override
	public List<WarrantyDto> generateAutoWarranties(DossierDataDto dossier) {
		DossierDataCoreDto dossierCore = dossierDataDtoMapper.convertToCoreDTO(dossier);
		generateDefaultWarranties(dossierCore);
		return dossierCore.getWarranties();
	}
	...
	public DossierDataDto validateDossierRequest(String uuid) {
		try {
			...
			updateDossierRequest(dossierRequestDto);
			return dossierDataDto;
		}
	...
	public List<RefUserDto>  getAllUsersFront(String uuid){
		DossierDataCoreDto dossierCore = dossierDataClient.retrieveByUuid(uuid);
		...
		UserCriteriasDto criteriasDto= UserCriteriasDto.builder()
				.codeProfessions(professions)
				.drCode(organization.getDrCode())
				.onlyInProfessions(Boolean.TRUE)
		...
		log.info("Fetching users with combined criteria: {}", criteriasDto);
		List<RefUserDto> users = fetchUsers(criteriasDto);
	...
	public List<RefUserDto> getAllUsersByHierarchy(String businessKey) {
		try{
			...
			if (Boolean.FALSE.equals(isUserInDirectory(assignee, connectedUser))) {
	...
	public Boolean isDossierAuthorizedReassign(String dossierUuid) {
		...
		return  opcStatus.contains(codeStatus) ;
	}
	...
	public ReassignmentRequestDto createReassignRequest(String uuid, String matricule, CommentDto comment) {
		...
		try{
			...
			ReassignmentRequestDto reassignmentRequestDto = ReassignmentRequestDto.builder()
					...
					.oldAssignee( currentAssign==null ? null: currentAssign.getMatricule())
			...
			TaskResult oldTask = new TaskResult();
			oldTask.setAssignee(currentAssign==null ? null:currentAssign.getFullName());
			TaskResult newTask = new TaskResult();
	...
	public DossierDataDto updateWarranties(DossierDataDto dossierDto) {
		...
		if(Arrays.asList(DossierStatus.DECS_ANR_RIDN.getCode(),
				DossierStatus.INFOS_COMP_AVRS_ANR_RIDN.getCode()).contains(dossierDto.getCodeStatus())){
	...
	private SearchRequest<DossierDataCriteria> prepareQueryParams(SearchRequest<DossierDataCriteria> searchRequest, List<TaskResult> userTasks){
		...
		switch (listType){
			case ALL_DOSSIERS_IN_POOL:
				...
				break;
			case ALL_DOSSIER_TRAITER_DSC:
				dossierDataCriteria.setNotInlistUuidDossier(getTaskUuids(userTasks));
				dossierDataCriteria.setInStatus(List.of(DossierStatus.DSC_OPC_GENERATION_PROCESSING.getCode()));
				break;
	...
	private List<String> getUserMarketCodes() {
		UserDto currentUser = UserHelper.getCurrentUser();
		List<String> userMarketCodes = new ArrayList<>( Optional.ofNullable(currentUser.getProfessionMarketsCodes())
                .orElse(Collections.emptyList()));
	...
	public void generateDefaultWarranties(DossierDataCoreDto dossier) {
		...
		boolean isCreatedBeforeTargetDate = dossier.getCreatedAt() != null && dossier.getCreatedAt().isBefore(LocalDateTime.of(2026, 6 , 15, 0, 0));
		if(isCreatedBeforeTargetDate){return;}
		...
		if (loanData != null) {
			defaultWarranties.add(warranty(WARRANTY_DECES_INVAL, CODE_DECES_INVAL, MoneyToWords.formatAmount(loanData.getLoanAmount())));
		}

		if (hasCaution && loanData != null) {
			guarantors.forEach(g -> defaultWarranties.add(warranty(WARRANTY_CAUTION_SOLIDAIRE,
					CODE_CAUTION_SOLIDAIRE, (g.getFirstName() + " " + g.getLastName()), MoneyToWords.formatAmount(loanData.getLoanAmount()))));
		}

		if (loanData != null && !("AQS".equals(loanData.getCodeLoanObject()) && "TRN".equals(loanData.getCodePropertyType()))) {
			defaultWarranties.add(warranty(WARRANTY_CONFORT_HAB,CODE_CONFORT_HAB, MoneyToWords.formatAmount(loanData.getLoanAmount())));
		}

		addWarrantiesForSpecificProducts(defaultWarranties, productCode);
		addPropertyWarranties(dossier, defaultWarranties);
	...
	private void addMarketWarranties(DossierDataCoreDto dossier, List<WarrantyDto> defaultWarranties, boolean hasEmprunteur) {
		...
		String country = dossier.getCustomerData().getCard().getCountry();
		if ((market.contains(MCH_01) || market.contains(MCH_09_PRI)) && "MAROC".equals(country.toUpperCase())) {
	...
	private void addPropertyWarranties(DossierDataCoreDto dossier, List<WarrantyDto> list) {
		...
		Optional.ofNullable(dossier.getBeneficiaries()).orElse(Collections.emptyList()).forEach(ben -> {
			boolean isEmprunteur = Boolean.TRUE.equals(ben.getIsBorrower());

			Map<Object, PropertyCoreDto> propertyMap = new HashMap<>();
			Optional.ofNullable(dossier.getPropertyData().getProperties()).orElse(List.of()).stream()
				.filter(Objects::nonNull)
				.forEach(p -> {
					if (p.getId() != null) {propertyMap.put(p.getId().toString(), p);}
					if (p.getUuid() != null) {propertyMap.put(p.getUuid(), p);}
				});

			Optional.ofNullable(ben.getRangs())
					.orElse(Collections.emptyList())
					.forEach(r -> {
						String key = r.getPropertyId() != null ? String.valueOf(r.getPropertyId()) : r.getPropertyUuid();
						PropertyCoreDto p = propertyMap.get(key);
						if (p != null) {
							if (isEmprunteur) {
								list.add(warranty(WARRANTY_HYPOTHEQUE, CODE_HYPOTHEQUE, r.getRang(), p.getLandCertificateNumber(), MoneyToWords.formatAmount(r.getWarrantyAmount())));
							}
							else {
								list.add(warranty(WARRANTY_CAUTION_HYPO, CODE_CAUTION_HYPO, r.getRang(), p.getLandCertificateNumber(), MoneyToWords.formatAmount(r.getWarrantyAmount())));
							}
						}
					});
		});
